...
def covPeriodic(loghyper=None, x=None, z=None):
    ...
    ell = numpy.exp(loghyper[0])         # characteristic length scale
    p   = numpy.exp(2.*loghyper[1])        # period
...
def covSumMat(covfunc, loghyper=None, x=None, R=None, w=None, z=None, Rstar=None):
    ...
    if loghyper == None:    # report number of parameters
        A = [Tools.general_old.feval(covfunc[0])]
        for i in range(1,len(covfunc)):
            A.append(Tools.general_old.feval(covfunc[i]))
        return A
    ...
    for i in range(1,len(covfunc)+1):
        no_param = Tools.general_old.feval(covfunc[i-1])  
    ...
    if z == None:   # compute covariance matrix
        ...
        for i in range(1,len(covfunc)+1):   # iteration over summand functions
            f = covfunc[i-1]
            if f=='kernels_old.covMatrix':
                A = A + w*Tools.general_old.feval(f, R)  # accumulate covariances
            else:
                tmp = Tools.general_old.feval(f, loghyper[int(v[i-1,0]):int(v[i-1,0])+int(v[i,0]),0], x)  # accumulate covariances
                A = A + Tools.general_old.feval(f, loghyper[int(v[i-1,0]):int(v[i-1,0])+int(v[i,0]),0], x)  # accumulate covariances
        ...
        if f=='kernels_old.covMatrix': 
            ...
            A = Tools.general_old.feval(f, loghyper[int(v[i-1,0]):int(v[i-1,0])+int(v[i,0]),0], x, int(j))
        ...
        for i in range(1,len(covfunc)+1):
            ...
            if f=='kernels_old.covMatrix':
                results = Tools.general_old.feval(f, R, Rstar)
                A = A + w*results[0]    # self covariances 
                B = B + w*results[1]    # cross covariances    
            else:
                results = Tools.general_old.feval(f, loghyper[int(v[i-1,0]):int(v[i-1,0])+int(v[i,0]),0], x, z)
...
def covSum(covfunc, loghyper=None, x=None, z=None):
    ...
    if z == None:   # compute covariance matrix
        ...
        for i in range(1,len(covfunc)+1): 
            ...
            if z<tmp:
                #j = z-v[i-1,0]; break       # j: which parameter in that covariance
                j = z-(tmp-v[i]); break       # j: which parameter in that covariance (BUG fixed 08/2012 )
                                            # i: which covariance function
        f = covfunc[i-1]
        # compute derivative
        A = Tools.general_old.feval(f,loghyper[int(v[i-1,0]):int(v[i-1,0])+int(v[i,0]),0], x, int(j))
...
def covProd(covfunc, loghyper=None, x=None, z=None):
    ...
    if z == None:   # compute covariance matrix
        ...
        for i in range(1,len(covfunc)+1):   # iteration over multiplicand functions
            f = covfunc[i-1]
            A *= Tools.general_old.feval(f,loghyper[int(v[i-1,0]):int(v[i-1,0])+int(v[i,0]),0], x)  # accumulate covariances
